...
智谱刚发布 GLM-5，阮一峰老师评测后表示"编程表现可圈可点，能够跟国外最新的旗舰模型放在一起"，并引用官方的定位——"Opus 4.6 与 GPT-5.3 的国产开源平替"。话音未落，MiniMax 又在 2 月 12 日推出了 M2.5——号称"1 万美金让 4 个 Agent 连续工作一年"，SWE-Bench Verified 跑到 80.2%，成本只有 Opus 的 1/10。而据可靠消息，Qwen 和 DeepSeek 的新模型也箭在弦上，发布只是时间问题。
...
|------|------|
| 2026.1.x | 智谱发布 GLM-5 |
| 2026.2.12 | MiniMax 发布 M2.5，SWE-Bench 80.2%，成本降到 Opus 的 1/10 |
| 即将 | Qwen、DeepSeek 新模型发布在即 |

每发布一个新模型，你是想：
- 等 OpenClaw 官方支持，刷 issue 等回复？
- 还是一句话就能用上？

MiniMax M2.5 的定位很有意思——"为真实世界生产力而生"，主打 Agent 场景下的编程、搜索、办公能力，而且成本极低（100 TPS 连续工作一小时只需 1 美金）。这类模型接入后，配合 Higress 的自动路由，可以根据任务复杂度智能选择模型，既省钱又高效。

## 自动路由：让 AI 自己选择最合适的模型

国产模型各有所长——GLM-5 架构能力强，MiniMax M2.5 性价比高，DeepSeek 推理能力突出。Higress 的自动路由可以根据任务类型智能调度：
